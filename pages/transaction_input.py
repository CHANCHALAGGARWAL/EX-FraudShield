...
def render(model, feature_columns, training_df):
    ...
    if submitted:
        ...
        pred, prob, reason, shap_list, X_in = common_utils.predict_and_explain_loaded(model, feature_columns, input_dict)
        ...
        if st.session_state.get("consent_save_audit", True):
            # Cross-platform unique transaction id using unix timestamp + short uuid
            ...
            try:
                common_utils.append_audit(entry)
